...
function Attention.model(loader, opt)
  ...
  if opt.att_model == 'lstm' then
    rnn_model = LSTM.lstm(loader.vocab_size, opt.att_rnn_size, opt.attention_num_layers, opt.dropout)
  elseif opt.att_model == 'rnn' then
    rnn_model = RNN.rnn(loader.vocab_size, opt.att_rnn_size, opt.attention_num_layers, opt.dropout)
  end
  if opt.att_model == 'lstm' then
    birnn_model = LSTM.lstm(loader.vocab_size, opt.att_rnn_size, opt.attention_num_layers, opt.dropout)
  elseif opt.att_model == 'rnn' then
    birnn_model = RNN.rnn(loader.vocab_size, opt.att_rnn_size, opt.attention_num_layers, opt.dropout)
  end
...
function Attention.clone_model(loader)
  ...
  print('cloning rnn')
  local clones_rnn = model_utils.clone_many_times(Attention.rnn, loader.seq_length)
  print('cloning ' .. loader.seq_length ..  ' rnns for each time series finished! ')
  local clones_birnn = nil
  print('cloning bidirectional rnn')
  clones_birnn = model_utils.clone_many_times(Attention.birnn, loader.seq_length)
  print('cloning ' .. loader.seq_length ..  ' birnns for each time series finished! ')
